...
#'  }
#'
plot.plsreg1 <- function(x, what = "variables", comps = c(1, 2), where = c("t", "t"), 
    cex = 1, col.xlabels = "#5592e3", col.ylabel = "#fe9429", yfont = 2, pos = NULL, 
    ...
    if (what == "variables") {
        ...
        arrows(x0 = rep(0, p), y0 = rep(0, p), x1 = x$cor.xyt[, k1], y1 = x$cor.xyt[, 
            k2], length = length, angle = angle, lwd = lwd, col = col.arrows)
        # text
        if (is.null(pos)) {
            neg <- x$cor.xyt[, k1] < 0
            pos = rep(4, p)
        ...
        text(x$cor.xyt[, k1], x$cor.xyt[, k2], labels = rownames(x$cor.xyt), font = fonts, 
