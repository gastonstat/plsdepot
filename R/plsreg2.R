...
#'  }
#'
plsreg2 <- function(predictors, responses, comps = 2, crosval = TRUE) {
    # ======================================================= checking arguments
    # ======================================================= check predictors
    ...
    if (crosval) {
        ...
        for (i in 2:h) Q2cum[i, ] = 1 - apply(PRESS[1:i, ]/RSS[1:i, ], 2, prod)
        Q2Gcum = Q2G
        for (i in 1:h) Q2Gcum[i] = 1 - prod((rowSums(PRESS)/rowSums(RSS[-h, ]))[1:i])
        Q2T = cbind(Q2, Q2G)
    ...
    dimnames(cor.tu) = list(paste(rep("t", h), 1:h, sep = ""), paste(rep("u", h), 
        1:h, sep = ""))
    dimnames(EV) = list(paste(rep("t", h), 1:h, sep = ""), c("R2X", "R2Xcum", "R2Y", 
        "R2Ycum"))
    ...
    # results
    structure(list(x.scores = Th, x.loads = Ph, y.scores = Uh, y.loads = Ch, cor.xt = cor.xt, 
        cor.yt = cor.yt, cor.xu = cor.xu, cor.yu = cor.yu, cor.tu = cor.tu, raw.wgs = Wh, 
        mod.wgs = Ws, std.coefs = Bs, reg.coefs = coeffs, y.pred = Y.hat, resid = resids, 
