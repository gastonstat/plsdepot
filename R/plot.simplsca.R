...
#'  }
#'
plot.simplsca <- function(x, what = "variables", comps = c(1, 2), where = c("t", 
    "t"), cex = 1, col.xlabels = "#5592e3", col.ylabels = "#fe9429", yfont = 2, pos = NULL, 
    offset = 0.1, col.xarrows = "#5b9cf255", col.yarrows = "#fe942955", lwd = 3, 
    length = 0, angle = 0, col.points = "#5592e3", pch = 21, pt.bg = "#5b9cf255", 
    show.names = FALSE, xpd = TRUE, xlab = NULL, ylab = NULL, main = NULL, col.main = "gray35", 
    cex.main = 1.2, col.axis = "gray40", show.grid = TRUE, col.grid = "gray95", ...) {
    # ======================================================= check inputs
    # =======================================================
    ...
    if (what == "variables") {
        ...
        mtext(seq(-1, 1, 0.5), side = 1, at = seq(-1, 1, 0.5), line = 0.5, col = col.axis, 
            cex = 0.8)
        axis(side = 2, labels = FALSE, lwd = 0, lwd.ticks = 1, col = "gray75")
        mtext(seq(-1, 1, 0.5), side = 2, at = seq(-1, 1, 0.5), line = 0.7, col = col.axis, 
            cex = 0.8, las = 2)
        box(col = "gray70")
        if (show.grid) 
        ...
        # arrows
        arrows(x0 = rep(0, p), y0 = rep(0, p), x1 = x$cor.xt[, k1], y1 = x$cor.xt[, 
            k2], length = length, angle = angle, lwd = lwd, col = c(rep(col.xarrows, 
            p)))
        arrows(x0 = rep(0, p), y0 = rep(0, p), x1 = x$cor.yu[, k1], y1 = x$cor.yu[, 
            k2], length = length, angle = angle, lwd = lwd, col = c(rep(col.yarrows, 
            q)))
        # text
        if (is.null(pos)) {
            negx <- x$cor.xt[, k1] < 0
        ...
        text(x$cor.xt[, k1], x$cor.xt[, k2], labels = rownames(x$cor.xt), col = c(rep(col.xlabels, 
            p)), cex = cex, pos = posx, offset = offset, ...)
        text(x$cor.yu[, k1], x$cor.yu[, k2], labels = rownames(x$cor.yu), font = yfont, 
            col = c(rep(col.ylabels, q)), cex = cex, pos = posy, offset = offset, 
            ...)
        
        if (is.null(main)) 
            main = "Circle of Correlations"
        mtext(main, side = 3, at = par("usr")[1], line = 1, adj = 0, cex = cex.main, 
            col = col.main)
        par(op)
    } else {
        # ======================================================= observations
        ...
        # where values
        ws = switch(as.character(tmp), `1` = cbind(x$x.scores[, k1], x$x.scores[, 
            k2]), `2` = cbind(x$x.scores[, k1], x$y.scores[, k2]), `3` = cbind(x$y.scores[, 
            k1], x$y.scores[, k2]))
        # axis labs
        axlabs = switch(as.character(tmp), `1` = c("t", "t"), `2` = c("t", "u"), 
            `3` = c("u", "u"))
        if (is.null(xlab)) 
        ...
        # plot
        plot(ws[, 1], ws[, 2], type = "n", axes = FALSE, xlab = xlab, ylab = ylab, 
            col.lab = col.axis)
        ...
        # points
        if (!show.names) {
            points(ws[, 1], ws[, 2], col = col.points, pch = pch, bg = pt.bg, lwd = 1.5, 
                cex = cex)
        } else {
            text(ws[, 1], ws[, 2], labels = rownames(x$x.scores), col = col.xlabels, 
                cex = cex, pos = pos, offset = offset, ...)
        }
        if (is.null(main)) 
