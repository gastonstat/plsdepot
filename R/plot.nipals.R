...
#'  }
#'
plot.nipals <- function(x, what = "variables", comps = c(1, 2), cex = 1, col.labels = "#5592e3", 
    pos = NULL, offset = 0.1, col.arrows = "#5b9cf255", lwd = 3.5, length = 0, angle = 0, 
    col.points = "#5592e3", pch = 21, pt.bg = "#5b9cf255", show.names = FALSE, xpd = TRUE, 
    xlab = NULL, ylab = NULL, main = NULL, col.main = "gray35", cex.main = 1.2, col.axis = "gray40", 
    ...
    if (what == "variables") {
        ...
        arrows(x0 = rep(0, p), y0 = rep(0, p), x1 = x$cor.xt[, k1], y1 = x$cor.xt[, 
            k2], length = length, angle = angle, col = col.arrows, lwd = lwd)
        # text
        if (is.null(pos)) {
            neg <- x$cor.xt[, k1] < 0
            pos = rep(4, p)
        ...
        text(x$cor.xt[, k1], x$cor.xt[, k2], labels = rownames(x$cor.xt), col = col.labels, 
        ...
        # ======================================================= observations
        # =======================================================
        plot(x$scores[, k1], x$scores[, k2], type = "n", axes = FALSE, xlab = xlab, 
        ...
        if (!show.names) {
            ...
            text(x$scores[, k1], x$scores[, k2], labels = rownames(x$scores), col = col.labels, 
                cex = cex, pos = pos, offset = offset, xpd = xpd, ...)
        }
        if (is.null(main)) 
