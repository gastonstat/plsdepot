...
interbat <- function(X, Y, scaled = TRUE) {
    ...
    # results
    res = list(values = eigs, x.scores = Xt, x.wgs = A, y.scores = Yu, y.wgs = B, 
        cor.xt = cor.xt, cor.yu = cor.yu, cor.xu = cor.xu, cor.yt = cor.yt, cor.tu = cor.tu, 
        R2X = R2X, R2Y = R2Y, com.xu = Com.XU, com.yt = Com.YT, statistic = stat_df)
    class(res) = "interbat"
    return(res)
}
